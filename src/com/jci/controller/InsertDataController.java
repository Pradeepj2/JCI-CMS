...
public class InsertDataController {
	...
	public ModelAndView viewFarmerList(final HttpServletRequest request) {
		...
		if (username == null) {
			...
			final List<FarmerRegistrationModel> allFarmersList = (List<FarmerRegistrationModel>) this.farmerRegistrationService
					.getAll();
	...
	public ModelAndView allOrganisationList(final HttpServletRequest request) {
		...
		final List<AddOrganizationModel> organisationList = (List<AddOrganizationModel>) this.addOrganisationService
				.getAll();
	...
	public ModelAndView ropeMaking(final HttpServletRequest request) {
		...
		if (username == null) {
			...
			try {
				...
				final List<RopeMakingModel> ropeList = (List<RopeMakingModel>) this.ropeMakingService
						.getAll(placeofactivity, regionId, zoneId);
	...
	public ModelAndView saveRopeMakingMid(final HttpServletRequest request,
		...
		try {
			...
			if (ropemadeInt > ropeUsedInt) {
				...
				redirectAttributes.addFlashAttribute("msg",
						(Object) "<div class=\"alert alert-danger\"><b>OOps!</b> Rope made must be greater than rope used. </div>\r\n");
			}
		} catch (Exception e) {
			System.out.println(e);
		}
	...
	//***************************************
	//Select districts which is not present in database for three columns
...
//		System.out.println("<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<");
		final Gson gson = new Gson();
		if(state_code == "") return "entry_derivativeprice.obj";
		return gson.toJson((Object) this.distric.getSpecificDistricts(state_code,crop_year,delivery_type));
	}
	...
	public ModelAndView FarmerRegistrationSave(final HttpServletRequest request,
			final RedirectAttributes redirectAttributes, @RequestParam("F_ID_PROF") final MultipartFile F_ID_PROF,
			@RequestParam("F_BANK_DOC") final MultipartFile F_BANK_DOC,
			@RequestParam("F_REG_FORM") final MultipartFile F_REG_FORM) {
		String username = (String) request.getSession().getAttribute("usrname");
		if (username == null) {
			return new ModelAndView("index");
		}
		final File theDir = new File(
				"E:\\Program Files\\Apache Software Foundation\\Tomcat 8.5\\webapps\\FarmerRegistration\\");
		if (!theDir.exists()) {
			theDir.mkdirs();
		}
		...
		try {
			...
			try {
				...
				if (!F_BANK_DOC.isEmpty()) {
					file = new File(
							"E:\\Program Files\\Apache Software Foundation\\Tomcat 8.5\\webapps\\FarmerRegistration\\bankdoc_"
									+ F_BANK_DOC.getOriginalFilename());
					try {
				...
				if (!F_ID_PROF.isEmpty()) {
					file = new File(
							"E:\\Program Files\\Apache Software Foundation\\Tomcat 8.5\\webapps\\FarmerRegistration\\idproof_"
									+ F_ID_PROF.getOriginalFilename());
					try {
						final OutputStream os = new FileOutputStream(file);
				...
				if (!F_REG_FORM.isEmpty()) {
					file = new File(
							"E:\\Program Files\\Apache Software Foundation\\Tomcat 8.5\\webapps\\FarmerRegistration\\regform_"
									+ F_REG_FORM.getOriginalFilename());
					try {
						final OutputStream os = new FileOutputStream(file);
			...
			if (accountBool && duplicateMobiile) {
				...
				redirectAttributes.addFlashAttribute("msg",
						(Object) "<div class=\"alert alert-danger\"><b>OOps!</b> Duplicate account number or mobile number or Identitiy Proof Number.</div>\r\n");
			}
		} catch (Exception e3) {
			System.out.println(e3);
			redirectAttributes.addFlashAttribute("msg",
					(Object) "<div class=\"alert alert-danger\"><b>Oops !</b> Error in saving record. Please try again</div>\r\n");
		}
	...
	public ModelAndView rawJutePaymentAndProcurement(final HttpServletRequest request) {
		...
		final List<String> farmerNo = (List<String>) this.rawJuteProcurAndPayService
				.getfarmerno((String) request.getSession().getAttribute("dpcId"));
	...
	public ModelAndView saveBalePreparation(final HttpServletRequest request,
			final RedirectAttributes redirectAttributes) {
		String username = (String) request.getSession().getAttribute("usrname");
		if (username == null) {
			return new ModelAndView("index");
	...
	public ModelAndView locatorDelete(final HttpServletRequest request, final RedirectAttributes redirectAttributes)
		...
		redirectAttributes.addFlashAttribute("msg",
				(Object) "<div class=\"alert alert-success\"><b>Success !</b> List deleted successfully.</div>\r\n");
	...
	public ModelAndView saveJBAList(final HttpServletRequest request, final RedirectAttributes redirectAttributes) {
		...
		try {
			...
			System.out.println(e);
			redirectAttributes.addFlashAttribute("msg",
					(Object) "<div class=\"alert alert-dange\"><b>Failure!</b> Error While saving record. Please try againn</div>\r\n");
		}
	...
	public ModelAndView JbaDelete(final HttpServletRequest request, final RedirectAttributes redirectAttributes)
			throws ParseException {
		String username = (String) request.getSession().getAttribute("usrname");
	...
	public ModelAndView rawJuteProcurementAndPaymentMid(final HttpServletRequest request,
			final RedirectAttributes redirectAttributes,
			@RequestParam("tallySlipdoc") final MultipartFile tallySlipdoc) {
		String username = (String) request.getSession().getAttribute("usrname");
		if (username == null) {
			return new ModelAndView("index");
		}
		...
		try {
			...
			if (basis.equalsIgnoreCase("commercial"))
				...
				basisNo = 1;
			final String result = this.DailyPurchasefService.findGradePriceJuteVariety(jutevariety, basisNo, cropyr,
					dpcid);
			...
			rawJuteProcAndPay.setGrade1xnetqty(prices[0] * Double.parseDouble(netquantity));
			rawJuteProcAndPay.setGrade2xnetqty(prices[1] * Double.parseDouble(netquantity));
			rawJuteProcAndPay.setGrade3xnetqty(prices[2] * Double.parseDouble(netquantity));
			rawJuteProcAndPay.setGrade4xnetqty(prices[3] * Double.parseDouble(netquantity));
			rawJuteProcAndPay.setGrade5xnetqty(prices[4] * Double.parseDouble(netquantity));
			rawJuteProcAndPay.setGrade6xnetqty(prices[5] * Double.parseDouble(netquantity));
			rawJuteProcAndPay.setGrade7xnetqty(prices[6] * Double.parseDouble(netquantity));
			rawJuteProcAndPay.setGrade8xnetqty(prices[7] * Double.parseDouble(netquantity));
			rawJuteProcAndPay.setAmountpayable((double) Float.parseFloat(amountPayable));
			if (farmerregno == null || farmerregno == "") {
				redirectAttributes.addFlashAttribute("msg",
						(Object) "<div class=\"alert alert-warning\"><b>Oops! </b> Please enter correct registartion number.</div>\r\n");
			} else {
	...
	public ModelAndView dailyPurchaseMid(final HttpServletRequest request,
			final RedirectAttributes redirectAttributes) {
		String username = (String) request.getSession().getAttribute("usrname");
		if (username == null) {
			return new ModelAndView("index");
		...
		try {
			...
			DailyPurchase.setGrade1xnetqty(prices[0] * Double.parseDouble(netquantity));
			DailyPurchase.setGrade2xnetqty(prices[1] * Double.parseDouble(netquantity));
			DailyPurchase.setGrade3xnetqty(prices[2] * Double.parseDouble(netquantity));
			DailyPurchase.setGrade4xnetqty(prices[3] * Double.parseDouble(netquantity));
			DailyPurchase.setGrade5xnetqty(prices[4] * Double.parseDouble(netquantity));
			DailyPurchase.setGrade6xnetqty(prices[5] * Double.parseDouble(netquantity));
			DailyPurchase.setGrade7xnetqty(prices[6] * Double.parseDouble(netquantity));
			DailyPurchase.setGrade8xnetqty(prices[7] * Double.parseDouble(netquantity));
	...
	public ModelAndView dailyPurchaseList(final HttpServletRequest request) {
		...
		String zoneId = (String) request.getSession().getAttribute("zoneId");
		String dpcid = (String) request.getSession().getAttribute("dpcId");
		final List<DailyPurchaseConfModel> purchaseList = (List<DailyPurchaseConfModel>) this.DailyPurchasefService
				.getAll(dpcid, regionId, zoneId);
	...
	public ModelAndView saveUserMid(final HttpServletRequest request, final RedirectAttributes redirectAttributes) {
		...
		try {
			...
			if (emailNotExist) {
				...
				redirectAttributes.addFlashAttribute("msg",
						(Object) "<div class=\"alert alert-warning\"><b>OOps!</b> Duplicate email id.</div>\r\n");
			}
		} catch (Exception ex) {
			ex.printStackTrace();
		}
	...
	public ModelAndView viewmarketArrival(final HttpServletRequest request) {
		...
		try {
			...
			final List<MarketArrivalModel> allmarketArrivalList = (List<MarketArrivalModel>) this.marketArrivalService
					.getAlldata(dpc_code, regionId, zoneId);
	...
	public ModelAndView viewFarmerLists(final HttpServletRequest request) {
		...
		try {
			...
			String regionId = (String) request.getSession().getAttribute("regionId");
			String zoneId = (String) request.getSession().getAttribute("zoneId");
			final List<FarmerRegModelDTO> allFarmersList = (List<FarmerRegModelDTO>) this.farmerRegService
					.verificationStatus(dcpid, regionId, zoneId);
	...
	public ModelAndView editFarmer(final HttpServletRequest request) {
		...
		try {
			final int id = Integer.parseInt(request.getParameter("id"));
			final List<ProgOfAssortmentModel> allbalePreparationList = (List<ProgOfAssortmentModel>) this.progOfAssortmentService
					.getAll();
	...
	public ModelAndView saveVerification(final HttpServletRequest request) {
		...
		try {
			 ...
			 * if(idProofType.equalsIgnoreCase(farmerIdProofTypeDb)) { idProofTypeFinal =
			 * idProofType;
			 * 
			...
			String dcpid = (String) request.getSession().getAttribute("dpcId");
			final List<FarmerRegModelDTO> allFarmersList = (List<FarmerRegModelDTO>) this.farmerRegService
					.verificationStatus(placeofactivity, regionId, zoneId);
	...
	public ModelAndView updateeditGradesPrice(final HttpServletRequest request,
			final RedirectAttributes redirectAttributes) {
		String username = (String) request.getSession().getAttribute("usrname");
		ModelAndView mv = new ModelAndView("editGradesPrice");
		try {
	...
	public ModelAndView deletemspGradesPriceList(final HttpServletRequest request,
			final RedirectAttributes redirectAttributes) {
		String username = (String) request.getSession().getAttribute("usrname");
		...
		try {
			...
			final List<MSPPriceCalculationModel> msppriceList = (List<MSPPriceCalculationModel>) this.mSPPriceCalculationService
					.getAll();
			mv.addObject("msppriceList", (Object) msppriceList);
			redirectAttributes.addFlashAttribute("msg",
					(Object) "<div class=\"alert alert-success\"><b>Success !</b> Data deleted successfully.</div>\r\n");
			if (username == null) {
				mv = new ModelAndView("index");
			}
			return new ModelAndView((View) new RedirectView("mspGradesPriceList.obj"));
	...
	public ModelAndView EditsaveFarmerRegistrationMid(final HttpServletRequest request,
			final RedirectAttributes redirectAttributes,
			@RequestParam("F_DOC_Mandate") final MultipartFile F_DOC_Mandate) {
		final ModelAndView mv = new ModelAndView();
		String username = (String) request.getSession().getAttribute("usrname");
		if (username == null) {
			return new ModelAndView("index");
		...
		try {
			...
			if (!F_DOC_Mandate.isEmpty()) {
				file = new File(
						"E:\\Program Files\\Apache Software Foundation\\Tomcat 8.5\\webapps\\FarmerRegistration\\mandate_"
								+ F_DOC_Mandate.getOriginalFilename());
				try {
					final OutputStream os = new FileOutputStream(file);
	...
	public ModelAndView saveDistributionoftallyslip(final HttpServletRequest request,
			final RedirectAttributes redirectAttributes) {
		String username = (String) request.getSession().getAttribute("usrname");
		if (username == null) {
			return new ModelAndView("index");
	...
	public ModelAndView viewDistributionoftallyslips(final HttpServletRequest request) {
		...
		try {
			...
			final List<DistributionoftallyslipModel> allDistributionoftallyslips = (List<DistributionoftallyslipModel>) this.distributionoftallyslipService
					.getAll(dpcId);
	...
	public ModelAndView viewUserRegistration(final HttpServletRequest request) {
		...
		if (username != null) {
			...
			String dpc_code = (String) request.getSession().getAttribute("dpcId");
			String dpcId = (String) request.getSession().getAttribute("dpcId");
			final List<UserRegistrationModel> allUserRegistration = (List<UserRegistrationModel>) this.UserRegistrationService
					.getAll(dpc_code, regionId, zoneId);
	...
	public ModelAndView viewProcurementList(final HttpServletRequest request) {
		...
		try {
			final List<RawJuteProcurementAndPayment> allProcurementList = (List<RawJuteProcurementAndPayment>) this.rawJuteProcurAndPayService
					.getAll();
	...
	public ModelAndView saveTallySlipMid(final HttpServletRequest request,
		...
		try {
			...
			final boolean procupdate = this.rawJuteProcurAndPayService.updateProcurementerror(status, is_verified,
					tallyNo, errors.trim(), region);
			// System.out.println(procupdate);
			if (procupdate) {
				this.verifyTallySlipService.submitform(verifyTallySlip);
				// mv.addObject("msg", (Object)"<div class=\"alert alert-success\"><b>Success
				// !</b> Tally slip verified successfully.</div>\r\n");
				redirectAttributes.addFlashAttribute("msg",
						(Object) "<div class=\"alert alert-success\"><b>Success !</b> Tally slip verified successfully.</div>\r\n");

			} else {
				// mv.addObject("msg", (Object)"<div class=\"alert alert-Failed\"><b>Fail to
				// save !</b> Tally slip verification failed.</div>\r\n");
				redirectAttributes.addFlashAttribute("msg",
						(Object) "<div class=\"alert alert-Failed\"><b>Success !</b>  Tally slip verification failed.</div>\r\n");
	...
	public ModelAndView viewCommercialCeilingPrice(final HttpServletRequest request) {
		...
		try {
			final List<CommercialJuteVarietyModel> commercialList = (List<CommercialJuteVarietyModel>) this.commercialJuteVarietyGradesPriceService
					.getAll();
	...
	public ModelAndView saveRulingMarketMid(final HttpServletRequest request,
			final RedirectAttributes redirectAttributes) {
		String username = (String) request.getSession().getAttribute("usrname");
		ModelAndView mv = new ModelAndView("rulingMarket");
		if (username == null) {
	...
	public ModelAndView bin(final HttpServletRequest request) {
		String username = (String) request.getSession().getAttribute("usrname");
		// final List<RawJuteProcurementAndPayment> binNumberList =
		// (List<RawJuteProcurementAndPayment>)this.rawJuteProcurAndPayService.getAll();
	...
	public ModelAndView binList(final HttpServletRequest request) {
		...
		final List<BatchIdentificationModel> batch = (List<BatchIdentificationModel>) this.batchService
				.getAll(placeofactivity, regionId, zoneId);
	...
	public ModelAndView viewbalePreparation(final HttpServletRequest request) {
		...
		try {
			...
			/*
			 * if(is_ho == 1) { viewBale =
			 * (List<BalePreparation>)this.balePrepareService.getAlldata(); }
			 */
	...
	public ModelAndView deleteBaleP(final HttpServletRequest request) {
		...
		try {
			...
			final List<BalePreparation> DeleteBalePreparation = (List<BalePreparation>) this.balePreparationService
					.getAll();
	...
	public ModelAndView deleteRopemaking(final HttpServletRequest request) {
		...
		try {
			...
			final List<RopeMakingModel> DeleteRopem = (List<RopeMakingModel>) this.ropeMakingService
					.getAll(placeofactivity, regionId, zoneId);
	...
	public ModelAndView deleteDpc(final HttpServletRequest request) {
		...
		try {
			...
			String dpcid = (String) request.getSession().getAttribute("dpcId");
			final List<DailyPurchaseConfModel> allDailyPurchase = (List<DailyPurchaseConfModel>) this.DailyPurchasefService
					.getAll(placeofactivity, regionId, zoneId);
	...
	public ModelAndView deletejuteprocurement(final HttpServletRequest request) {
		...
		try {
			...
			final List<RawJuteProcurementAndPayment> DeletejuteProcu = (List<RawJuteProcurementAndPayment>) this.rawJuteProcurAndPayService
					.getAll();
	...
	public ModelAndView updateJuteProcurement(final HttpServletRequest request,
			final RedirectAttributes redirectAttributes) {
		String username = (String) request.getSession().getAttribute("usrname");
		if (username == null) {
			return new ModelAndView("index");
	...
	public ModelAndView deletedistributiontally(final HttpServletRequest request) {
		...
		try {
			...
			final List<DistributionoftallyslipModel> Deletedistributiontally = (List<DistributionoftallyslipModel>) this.distributionoftallyslipService
					.getAll(dpcId);
	...
	public ModelAndView deletecommercialprice(final HttpServletRequest request) {
		...
		try {
			...
			final List<CommercialJuteVarietyModel> deleteCommercialList = (List<CommercialJuteVarietyModel>) this.commercialJuteVarietyGradesPriceService
					.getAll();
	...
	public ModelAndView deleteRulingMarket(final HttpServletRequest request,
			final RedirectAttributes redirectAttributes) {
		String username = (String) request.getSession().getAttribute("usrname");
		ModelAndView mv = new ModelAndView("viewRulingMarket");
		if (username == null) {
	...
	public ModelAndView updateBalePreparation(final HttpServletRequest request,
			final RedirectAttributes redirectAttributes) {
		String username = (String) request.getSession().getAttribute("usrname");
		...
		try {
			...
			// System.out.println(">>>>>data>>>>>>>>>>>baleid:" + baleid +
			// "place_of_packing:" + place_of_packing + "crop_year:" + crop_year + "bin_no:"
			// + bin_no + "basis:" + basis + "jute_variety: " + jute_variety +
			// "slip_no_from:" + slip_no_from + "slip_no_from:" + slip_no_from +
			// "slip_no_to:" + slip_no_to + "bale_no:" + bale_no + "jute_grade:" +
			// jute_grade);
			...
			redirectAttributes.addFlashAttribute("msg",
					(Object) "<div class=\"alert alert-danger\"><b>OOps!</b> Error in record saving. </div>\r\n");
	...
	public ModelAndView updateRopeMakingMid(final HttpServletRequest request,
			final RedirectAttributes redirectAttributes) {
		String username = (String) request.getSession().getAttribute("usrname");
		if (username == null) {
			return new ModelAndView("index");
	...
	public ModelAndView updateDailyPurchase(final HttpServletRequest request,
			final RedirectAttributes redirectAttributes) {
		String username = (String) request.getSession().getAttribute("usrname");
	...
	public String findJuteOnBasis(final HttpServletRequest request) {
		...
		try {
			result = (List<String>) this.rawJuteProcurAndPayService
					.findJuteOnBasis(Integer.parseInt(request.getParameter("msp_no")));
	...
	public String findGradeOnJuteVariety(final HttpServletRequest request) {
		final Gson gson = new Gson();
		final List<String> result = (List<String>) this.rawJuteProcurAndPayService.findGradeOnJuteVariety(
				request.getParameter("variety"), Integer.parseInt(request.getParameter("basis_no")));
		return gson.toJson((Object) result);
	}
	...
	public String findGradeOfMSP(final HttpServletRequest request) {
		...
		final List<String> result = (List<String>) this.mSPPriceCalculationService
				.findGradeOfMSP(request.getParameter("variety"), Integer.parseInt(request.getParameter("basis_no")));
		return gson.toJson((Object) result);
	}
	...
	public ModelAndView editdistributiontally(final HttpServletRequest request,
			final RedirectAttributes redirectAttributes) {
		String username = (String) request.getSession().getAttribute("usrname");
		...
		try {
			final String id = request.getParameter("id");
			final DistributionoftallyslipModel distributiontallyslip = this.distributionoftallyslipService
	...
	public ModelAndView updateDistributionoftallyslip(final HttpServletRequest request,
			final RedirectAttributes redirectAttributes) {
		String username = (String) request.getSession().getAttribute("usrname");
	...
	public ModelAndView editcommercialprice(final HttpServletRequest request,
			final RedirectAttributes redirectAttributes) {
		String username = (String) request.getSession().getAttribute("usrname");
		...
		try {
			final String id = request.getParameter("id");
			final CommercialCeilingPriceIntimationModel commercialCeilingPrice = this.commercialCeilingPriceIntimationService
					.find(Integer.parseInt(id));
	...
	public ModelAndView updateCommercialCeilingPriceIntimation(final HttpServletRequest request,
			final RedirectAttributes redirectAttributes) {
		String username = (String) request.getSession().getAttribute("usrname");
	...
	public ModelAndView saveGradePriceOfMSP(final HttpServletRequest request,
			final RedirectAttributes redirectAttributes) {
		String username = (String) request.getSession().getAttribute("usrname");
		...
		try {
			...
			if (msp > 0) {
				...
				redirectAttributes.addFlashAttribute("msg",
						(Object) "<div class=\"alert alert-danger\"><b>Not saved !</b> Record Not Saved.</div>\r\n");
			}
			if (dubjuteVarietyB) {
				...
				redirectAttributes.addFlashAttribute("msg",
						(Object) "<div class=\"alert alert-danger\"><b>OOps!</b> Duplicate jute variety</div>\r\n");
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public ModelAndView editRulingMarket(final HttpServletRequest request,
			final RedirectAttributes redirectAttributes) {
		String username = (String) request.getSession().getAttribute("usrname");
		if (username == null) {
			return new ModelAndView("index");
	...
	public ModelAndView saveGradePriceOfCommercial(final HttpServletRequest request,
			final RedirectAttributes redirectAttributes) {
		String username = (String) request.getSession().getAttribute("usrname");
		ModelAndView mv = new ModelAndView("CommercialJuteVarietyGradesPrice");
		if (username == null) {
	...
	public String findGradePriceJuteVariety(final HttpServletRequest request, final HttpSession session) {
		...
		final String dpcid = (String) request.getSession().getAttribute("dpcId");
		final Gson gson = new Gson();
		final List<String> result = (List<String>) this.rawJuteProcurAndPayService.findGradePriceJuteVariety(
				request.getParameter("variety"), Integer.parseInt(request.getParameter("basis_no")),
				request.getParameter("cropyr"), dpcid);
	...
	public ModelAndView updateRulingMarket(final HttpServletRequest request,
			final RedirectAttributes redirectAttributes) {
		String username = (String) request.getSession().getAttribute("usrname");
		ModelAndView mv = new ModelAndView("editRulingMarket");
		try {
	...
	public ModelAndView saveBale(final HttpServletRequest request, final RedirectAttributes redirectAttributes) {
		...
		try {
			...
			if (toCheckSlipNoInt > fromCheckSlipNoInt) {
				...
				redirectAttributes.addFlashAttribute("msg",
						(Object) "<div class=\"alert alert-success\"><b>Success !</b> Record created successfully.</div>\r\n");
				return new ModelAndView((View) new RedirectView("balePreparation.obj"));
			}
			redirectAttributes.addFlashAttribute("msg",
					(Object) "<div class=\"alert alert-danger\"><b>OOps!</b> Error in record saving. </div>\r\n");
	...
	public String GetDpcName(final HttpServletRequest request) {
		...
		final Gson gson = new Gson();
		final List<String> DaysCount = (List<String>) this.batchService
				.GetDpcNamefromId(Integer.parseInt(request.getParameter("dpcCode")));
	...
	public String transectionDetails(final HttpServletRequest request) {
		final String details = this.verifyTallySlipService.GettransectionDetails(request.getParameter("tallyslipNo"),
	...
	public ModelAndView viewDisputedTallySlipList(final HttpServletRequest request) {
		...
		List<VerifyTallySlip> verifyList = (List<VerifyTallySlip>) this.verifyTallySlipService.getAll("RMD", region,
				role_type);
		mv.addObject("verifyTallySliList", (Object) verifyList);
	...
	public ModelAndView tallyapproval(final HttpServletRequest request) {
		...
		final List<RawJuteProcurementAndPayment> juteList = (List<RawJuteProcurementAndPayment>) this.rawJuteProcurAndPayService
				.jutelistbystatus("ROV", request);
	...
	public String findBinno(final HttpServletRequest request) {
		...
		final List<String> result = (List<String>) this.ropeMakingService.findBinno(request.getParameter("cropyr"),
				request.getParameter("dpcid"));
		return gson.toJson((Object) result);
	}
	...
	public String InsertIntoTablePurchaseMapping(HttpServletRequest request) {
		...
		Gson gson = new Gson();
		List<String> DaysCount = batchService.InsertToBinPurchaseMapping(dateOfPurch, dpcCode, Basis, JuteVariety,
				CropYr, binNo, totNetQty, totGarsat, totValue);
		return gson.toJson(DaysCount);
	}
	...
	public String updatedpaymentstatus(final HttpServletRequest request, final RedirectAttributes redirectAttributes,
			HttpSession session) {
		String a = "";
		try {
			String username = (String) request.getSession().getAttribute("usrname");
			// String path1 ="E:\\Program Files\\Apache Software Foundation\\Tomcat
			// 8.5\\webapps\\TallySlipPayments\\";
			String path1 = "D:\\";
			...
			String jciref = "";
			String[] columns = { "Amount", "Debit A/C No", "Beneficiary IFSC code", "Beneficiary A/C No", "A/C type",
					"Beneficiary Name", "Beneficiary Branch", "JCI Ref", "Sender", "Beneficiary Bank", "Purchase Date",
					"UTR No", "Date" };
			...
			fileOut.close();
			workbook.close();
			String subject = "NEFT Advice Sheet: " + usrname + "-Rs." + totalamount + ", " + totalamount
					+ " will denote summation of tally slip amount within the sheet.";
			String toEmail = "";
			...
			if (roho.equalsIgnoreCase("RO")) {
				...
				InternetAddress[] toAddresses = { new InternetAddress(FA_approver_email), new InternetAddress(usermail),
						new InternetAddress("vishal.vishwakarma@cyfuture.com"),
						new InternetAddress("animesh.anand@cyfuture.com") };
				toEmail = FA_approver_email + ", " + usermail
						+ ", vishal.vishwakarma@cyfuture.com, animesh.anand@cyfuture.com";
	...
	public ModelAndView popupimage(HttpServletRequest request, HttpSession session) {
		...
		final List<ImageVerificationModel> images = (List<ImageVerificationModel>) verifyTallySlipService
				.getImages(tallyNo);
	...
	public ModelAndView viewVerifiedTallySlipListRM(final HttpServletRequest request) {
		...
		final List<VerifyTallySlip> verifyList = (List<VerifyTallySlip>) this.verifyTallySlipService.getAllforRM("RMZM",
				region);
	...
	public ModelAndView viewVerifiedTallySlipListZM(final HttpServletRequest request) {
		...
		final List<VerifyTallySlip> verifyList = (List<VerifyTallySlip>) this.verifyTallySlipService.getAllforZM("RMZM",
				region_zone);
	...
	public ModelAndView uploadexcel(final HttpServletRequest request, final RedirectAttributes redirectAttributes,
			@RequestParam("excelsheet") final MultipartFile excelsheet)
	...
	public ModelAndView updateFarmerDetails(final HttpServletRequest request,
			final RedirectAttributes redirectAttributes, @RequestParam("F_ID_PROF") final MultipartFile F_ID_PROF,
			@RequestParam("F_BANK_DOC") final MultipartFile F_BANK_DOC,
			@RequestParam("F_REG_FORM") final MultipartFile F_REG_FORM) {
		String username = (String) request.getSession().getAttribute("usrname");
		...
		try {
			...
			System.out.println(e3);
			redirectAttributes.addFlashAttribute("msg",
					(Object) "<div class=\"alert alert-danger\"><b>Oops !</b> Error in updating record. Please try again</div>\r\n");
		}
